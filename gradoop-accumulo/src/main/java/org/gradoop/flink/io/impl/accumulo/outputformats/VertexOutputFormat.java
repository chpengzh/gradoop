 ...
 */
public class VertexOutputFormat extends AbstractAccumuloOutputFormat<Vertex> {

  /**
   *
   * @param config gradoop accumulo config
   */
  public VertexOutputFormat(GradoopAccumuloConfig config) {
    super(config);
  }

  @Override
  protected BatchWriter getWriter(Connector conn) throws TableNotFoundException {
    return conn.createBatchWriter("gradoop.vertex", new BatchWriterConfig());
  }

  @Override
  protected void write(BatchWriter writer, Vertex record) throws MutationsRejectedException {
