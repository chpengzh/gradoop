...
/**
 * Vertex source row iterator
 */
public class VertexSourceRowIterator implements ClosableIterator<VertexSourceRow> {
   ...
   * Element cache size
   */
  private List<VertexSourceRow> cache = new ArrayList<>();
   ...
   */
  public VertexSourceRowIterator(
  ...
  public boolean hasNext() {
    if (!cache.isEmpty()) {
      ...
      while (iterator.hasNext() && cache.size() < cacheSize) {
        VertexSourceRow next = handler.readFromVertex(iterator.next());
        cache.add(next);
      }
      return hasNext();
